...
function NotificationCard({ notification }: {
    ...
    return (
        <>
            <div className="py-4" >
                <div className="">
                    <Link href={`/profile/${encodeURIComponent(author)}`} className="flex items-center gap-2 mb-2">
                        {
                            notification.event === "liked" &&
                            <Image src='/images/icons8-love-48.png' height={24} width={24} alt="like" />}
                            {
                            notification.event === "commented" &&
                            <Image src='/images/icons8-comment-24.png' height={24} width={24} alt="comment" />
                            }
                        {
                    ...
                    <p className="text-sm">
                        ...
                        <Link className="text-gray-600 font-medium" href={`/inkspire/${encodeURIComponent(id)}`}>{notification?.articleTitle}</Link>
                    </p>
                </div>
            </div>
        </>);
